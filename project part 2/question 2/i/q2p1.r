...
start_date <- pmin(as.Date("01-Jul-13", format="%d-%b-%y"))

end_date <- pmin(as.Date("31-Jul-13", format="%d-%b-%y"))
...
targeted_dates_ccl <- subset(data_set, as.Date(data_set$TIMESTAMP, format="%d-%b-%y") >= start_date & as.Date(data_set$TIMESTAMP, format="%d-%b-%y") <= end_date & data_set$SYMBOL == "CCL")
