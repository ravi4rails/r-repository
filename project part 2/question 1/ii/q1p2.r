...
fyq_start_date <- pmin(as.Date("01-Jul-14", format="%d-%b-%y"))

fyq_end_date <- pmin(as.Date("30-Sep-14", format="%d-%b-%y"))


second_fyq_month_one <- pmin(as.Date("01-Jul-14", format="%d-%b-%y"))
second_fyq_month_two <- pmin(as.Date("01-Aug-14", format="%d-%b-%y"))
second_fyq_month_three <- pmin(as.Date("01-Sep-14", format="%d-%b-%y"))
...
targeted_dates_reliance <- subset(data_set_two, as.Date(data_set_two$TIMESTAMP, format="%d-%b-%y") >= fyq_start_date & as.Date(data_set_two$TIMESTAMP, format="%d-%b-%y") <= fyq_end_date)
